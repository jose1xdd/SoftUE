...
public interface TestRepository extends JpaRepository<Test, Integer> {
    ...
    @Query(value = "SELECT * FROM test WHERE codigo_estudiante = :codigo AND fecha_creacion = :fecha ORDER BY id DESC LIMIT 1", nativeQuery = true)
    ...
    @Query(value = "SELECT t.* FROM test t JOIN estudiante e ON(t.codigo_estudiante = e.codigo) " +
                    "JOIN usuario u ON e.codigo = u.codigo " +
                    "WHERE ((:codigoEstudiante IS NULL) OR (e.codigo = :codigoEstudiante)) AND " +
                    "((:cursoEstudiante IS NULL) OR (e.curso = :cursoEstudiante)) AND " +
