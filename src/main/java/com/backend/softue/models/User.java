...
public class User {
    ...
    @Column(nullable = false)
    @Pattern(regexp = "^(?=.*[A-Z])(?=.*\\d)(?=.*[\\x21-\\x2F\\x3A-\\x40\\x5B-\\x60\\x7B-\\x7E]).{6,}$",message = "Error: La contraseña que ha ingresado debe cumplir con los siguientes requisitos minimos : almenos 6 caracteres ,una letra en mayuscula, un numero y un caracter especial")
